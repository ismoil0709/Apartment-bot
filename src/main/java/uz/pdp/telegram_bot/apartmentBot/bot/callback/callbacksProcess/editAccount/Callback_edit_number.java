...
public class Callback_edit_number {
    public static void process(Update update, TelegramLongPollingBot bot) {
        if (update.hasMessage()) {
            if (Regex.phoneNumberValidator(update.getMessage().getText())) {
                Client client = ClientRepoImpl.getInstance().get(UpdateProcessor.extractChatId(update));
                client.setNumber(update.getMessage().getText());
                ...
                GetAndSetStates.setWriteState(update, StateForWrite.WRITE_EDIT_NUMBER);
            }
        }
    }
    ...
    public static void sendMessage(Update update, TelegramLongPollingBot bot) {
        try {
            ...
            GetAndSetStates.setWriteState(update, StateForWrite.WRITE_EDIT_NUMBER);
            GetAndSetStates.setBackButtonState(update, StateForBack.EDIT_NUMBER);
        } catch (TelegramApiException e) {
            throw new RuntimeException(e);
        }
    }
