...
public class CallBack_Next {
    private static final Logger log = LoggerFactory.getLogger(CallBack_Next.class.getName());
    private static void getList(Update update, TelegramLongPollingBot bot, List<Apartment> apartments) {
        if (SendApartment.index >= apartments.size() - 1) {
            try {
    ...
    public static void process(Update update, TelegramLongPollingBot bot){
        ...
        switch (state){
            case PRICE -> CallBack_Next.getList(update,bot,Callback_search_with_price.sorted);
            case ROOMS -> CallBack_Next.getList(update,bot, Callback_search_with_number_of_rooms.sorted);
            case CITY -> CallBack_Next.getList(update,bot, Callback_search_with_city.sorted);
            case RECENTLY_UPLOADED -> CallBack_Next.getList(update,bot, ApartmentRepoImpl.getInstance().list());
            case MY_APARTMENTS -> {
                Client client = ClientRepoImpl.getInstance().get(UpdateProcessor.extractChatId(update));
                CallBack_Next.getMyList(update,bot,client.getApartment());
            }
        }
    }
}
